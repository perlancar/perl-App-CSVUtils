...
_
    ...
    after_close_input_files => sub {
        ...
        if ($r->{util_args}{key}) {
            ...
            for my $row (@{ $r->{util_args}{hash} ? $r->{rows_as_hashref} : $r->{rows} }) {
                local $_ = $row;
        ...
        if ($r->{util_args}{by_code} || $r->{util_args}{by_sortsub}) {
            ...
            if ($sort_indices) {
                my @sorted_indices = sort { local $main::a=$a; local $main::b=$b; $code->($main::a,$main::b) } 0..$#{$r->{rows}};
                $r->{rows} = [map {$r->{rows}[$_]} @sorted_indices];
            } else {
                $r->{rows} = [sort { local $main::a=$a; local $main::b=$b; $code->($main::a,$main::b) } @{$r->{rows}}];
            }
